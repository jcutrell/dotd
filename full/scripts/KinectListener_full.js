...
function KinectListener(skeletonBoxId){
	...
	var timePoints = {
		openDoorStart: 41,
		openDoorEnd: 48, //ghost breaks in
		takeHandStart_angry: 79,
		takeHandEnd_angry: 86, //ghost begins throwing objects
		objectDodge1: 91,
		objectDodge: 97,
		ghostBurns_dodge1: 108,
		openingDoor: 108,
		takeHandStart_happy: 121, 
		takeHandEnd_happy: 126, //take ghosts hand
		peekabooStart: 132,
		peekabooEnd: 143, //failed peek-a-boo, ghost attacks
		peekabooFailEnd: 157, //fades to black
		peekabooSuccessStart: 172,
		peekabooSuccessEnd: 187	
	}
	...
	function setFadeBlack(direction){
		if(direction == "out"){
			...
			setTimeout(function(){resetExperience();}, 4000);
		}
	...
	function updateVideoTime(event){
		...
		if(videoTime > 14 && videoTime < 51 && !openedDoor){ //time to open door has expired
			...
			setTimeout(function(){setFadeBlack("out");}, 4000);
		}
	...
	function setUserVisibility(value){
		...
		if(userIsVisibleToKinect){
			...
			if(!experienceHasBegun){
				output("I see you!");
				beginExperienceTimer = setTimeout(function(){output("begin"); experienceHasBegun = true; mainVideo.play();}, 3000);
			}
		}
		else{
